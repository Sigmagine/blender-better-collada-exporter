...
class DaeExporter:
	...
	class Vertex:
		...
		__slots__ = ("vertex", "normal", "tangent", "bitangent", "color", "uv", "uv2", "bones", "weights")
	...
	def export_image(self, image):
		...
		if (self.config["use_copy_images"]):
			...
			if os.path.isfile(imgpath):
				...
				imgpath = os.path.join("Textures", os.path.basename(imgpath))
			else:
				...
				if not os.path.isfile(dstfile):
					image.save()
				imgpath = os.path.join("Textures", os.path.basename(image.filepath))
	...
	def export_material(self, material):
		...
		principledBSDF = PrincipledBSDFWrapper(material) if material else None

		if principledBSDF:
			
			#-------------------------------------------------------------------------------------------------
			#export used samplers
			#-------------------------------------------------------------------------------------------------
			
			
			
			emission_texture = ShaderImageTextureWrapper(
				principledBSDF, principledBSDF.node_principled_bsdf,
				principledBSDF.node_principled_bsdf.inputs["Emission"],
				grid_row_diff=0,
				) if principledBSDF.use_nodes and principledBSDF.node_principled_bsdf is not None else None
			emission_texture_sampler = self.export_sampler(emission_texture)
			
			base_color_texture = principledBSDF.base_color_texture
			base_color_texture_sampler = self.export_sampler(base_color_texture)
			
			specular_texture = principledBSDF.specular_texture
			specular_texture_sampler = self.export_sampler(specular_texture)
			
			metallic_texture = principledBSDF.metallic_texture
			metallic_texture_sampler = self.export_sampler(metallic_texture)
		...
		if principledBSDF:
			...
			if emission_texture and emission_texture.image:
				self.writel(S_FX, 6, "<texture texture=\"{}\" texcoord=\"CHANNEL1\"/>".format(emission_texture_sampler))
			elif principledBSDF.use_nodes and principledBSDF.node_principled_bsdf is not None:  
				self.writel(S_FX, 6, "<color>{}</color>".format(numarr_alpha(rgba_to_rgb(principledBSDF.node_principled_bsdf.inputs["Emission"].default_value), 1.0)))	 
			self.writel(S_FX, 5, "</emission>")
			...
			self.writel(S_FX, 6, "<color>{}</color>".format(numarr_alpha((0.0,0.0,0.0), 1.0)))# self.scene.world.ambient_color and material.ambient are removed too
			self.writel(S_FX, 5, "</ambient>")
			"""

			self.writel(S_FX, 5, "<diffuse>")
			if base_color_texture and base_color_texture.image:
				self.writel(S_FX, 6, "<texture texture=\"{}\" texcoord=\"CHANNEL1\"/>".format(base_color_texture_sampler))
			else:
				self.writel(S_FX, 6, "<color>{}</color>".format(numarr_alpha(principledBSDF.base_color, 1.0)))
			self.writel(S_FX, 5, "</diffuse>")
			...
			if specular_texture and specular_texture.image:
				self.writel(S_FX, 6, "<texture texture=\"{}\" texcoord=\"CHANNEL1\"/>".format(specular_texture_sampler))
			else:
			...
			self.writel(S_FX, 6, "<float>{}</float>".format(50))# material.specular_hardness is removed too
			self.writel(S_FX, 5, "</shininess>")
			"""
			
			
			if metallic_texture and metallic_texture.image:
				self.writel(S_FX, 5, "<reflective>")
				self.writel(S_FX, 6, "<texture texture=\"{}\" texcoord=\"CHANNEL1\"/>".format(metallic_texture_sampler))
				self.writel(S_FX, 5, "</reflective>")
			else:
				self.writel(S_FX, 5, "<reflectivity>")
				self.writel(S_FX, 6, "<float>{}</float>".format(principledBSDF.metallic))
				self.writel(S_FX, 5, "</reflectivity>")
			...
			# This one is not official but seems to be used in Collada Maya ...
			self.writel(S_FX, 5, "<double_sided>")
			self.writel(S_FX, 6, "<bool sid=\"double_sided\">{}</bool>".format(bool(not material.use_backface_culling)))
			self.writel(S_FX, 5, "</double_sided>")
			...
			if normalmap_texture and normalmap_texture.image:
				self.writel(S_FX, 6, "<bump bumptype=\"NORMALMAP\">")
				self.writel(S_FX, 7, "<texture texture=\"{}\" texcoord=\"CHANNEL1\"/>".format(normalmap_texture_sampler))
				self.writel(S_FX, 6, "</bump>")

			self.writel(S_FX, 5, "</technique>")
			
			self.writel(S_FX, 5, "<technique profile=\"GOOGLEEARTH\">")
			self.writel(S_FX, 6, "<double_sided>{}</double_sided>".format(int(not material.use_backface_culling)))
			self.writel(S_FX, 5, "</technique>")
	...
	def export_mesh(self, node, armature=None, skeyindex=-1, skel_source=None, custom_name=None):
		...
		if (skeyindex == -1 and mesh.shape_keys is not None and len(mesh.shape_keys.key_blocks) and self.config["use_shape_key_export"]):
			values = []
			morph_targets = []
			md = None
			...
			self.writel(S_MORPH, 4, "<IDREF_array id=\"{}-morph-targets-array\" "
			...
			self.writel(S_MORPH, 5, "<accessor source=\"#{}-morph-targets-array\" "
			...
			self.writel(S_MORPH, 4, "<technique_common>")
			self.writel(S_MORPH, 5, "<accessor source=\"#{}-morph-weights-array\" "
				"count=\"{}\" stride=\"1\">".format(mid, len(morph_targets) - 1))
			...
			self.writel(S_MORPH, 4, "<input semantic=\"MORPH_TARGET\" "
				"source=\"#{}-morph-targets\"/>".format(mid))
			self.writel(S_MORPH, 4, "<input semantic=\"MORPH_WEIGHT\" "
		...
		if has_tangents and len(mesh.uv_layers):
			try:
				...
				self.operator.report({"WARNING"}, "CalcTangets failed for mesh \"{}\", no tangets will be "
				...
				if armature is not None:
					...
					if (wsum == 0.0):
						if not self.wrongvtx_report:
							self.operator.report(  {"WARNING"}, "Mesh for object \"{}\" has unassigned "
								"weights. This may look wrong in exported "
								"model.".format(node.name))
							self.wrongvtx_report = True

						# TODO: Explore how to deal with zero-weight bones, #	   which remain local
						v.bones.append(0)
						v.weights.append(1)
		...
		self.writel(S_GEOM, 4,
			"<input semantic=\"POSITION\" source=\"#{}-positions\"/>".format(meshid))
		...
		for m in surface_indices:
			...
			if (mat is not None):
				...
				self.writel(S_GEOM, 3, "<{} count=\"{}\" material=\"{}\">".format(prim_type, int(len(indices)), matref))  # TODO: Implement material
				mat_assign.append((mat, matref))
			else:
				self.writel(S_GEOM, 3, "<{} count=\"{}\">".format(prim_type, int(len(indices))))  # TODO: Implement material

			self.writel(S_GEOM, 4, "<input semantic=\"VERTEX\" "
				"source=\"#{}-vertices\" offset=\"0\"/>".format(meshid))
			self.writel(S_GEOM, 4, "<input semantic=\"NORMAL\" "
				"source=\"#{}-normals\" offset=\"0\"/>".format(meshid))

			for uvi in range(uv_layer_count):
				self.writel(S_GEOM, 4, "<input semantic=\"TEXCOORD\" source=\"#{}-texcoord-{}\" "
					"offset=\"0\" set=\"{}\"/>".format(meshid, uvi, uvi))

			if (has_colors):
				self.writel(S_GEOM, 4, "<input semantic=\"COLOR\" "
					"source=\"#{}-colors\" offset=\"0\"/>".format(meshid))
			if (has_tangents):
				self.writel(S_GEOM, 4, "<input semantic=\"TEXTANGENT\" "
					"source=\"#{}-tangents\" offset=\"0\"/>".format(meshid))
				self.writel(S_GEOM, 4, "<input semantic=\"TEXBINORMAL\" "
		...
		if (armature is not None and (skel_source is not None or skeyindex == -1)):
			...
			self.writel(S_SKIN, 4, "<input semantic=\"JOINT\" source=\"#{}-joints\"/>".format(contid))
			self.writel(S_SKIN, 4, "<input semantic=\"INV_BIND_MATRIX\" "
				"source=\"#{}-bind_poses\"/>".format(contid))
			self.writel(S_SKIN, 3, "</joints>")
			self.writel(S_SKIN, 3, "<vertex_weights count=\"{}\">".format(len(vertices)))
			self.writel(S_SKIN, 4, "<input semantic=\"JOINT\" "
				"source=\"#{}-joints\" offset=\"0\"/>".format(contid))
			self.writel(S_SKIN, 4, "<input semantic=\"WEIGHT\" "
	...
	def export_mesh_node(self, node, il):
		...
		if (armcount > 0 and not armature):
			self.operator.report({"WARNING"}, "Object \"{}\" has armature modifier, but is not a child of "
	...
	def export_armature_bone(self, bone, il, si):
		is_ctrl_bone = (self.config["use_exclude_ctrl_bones"] and
			(bone.name.startswith("ctrl") or bone.use_deform == False))
		if (bone.parent is None and is_ctrl_bone is True):
			self.operator.report({"WARNING"}, "Root bone cannot be a control bone:"+bone.name)
			is_ctrl_bone = False
		...
		if (is_ctrl_bone is False):
			...
			if (bone.name in self.used_bones):
				if (self.config["use_anim_action_all"]):
					self.operator.report({"WARNING"}, "Bone name \"{}\" used in more than one "
						"skeleton. Actions might export wrong.".format(bone.name))
			else:
			...
			self.writel(S_NODES, il, "<node id=\"{}\" sid=\"{}\" name=\"{}\" "
	...
	def export_camera_node(self, node, il):
		...
		if (camera.type == "PERSP"):
			...
			self.writel(S_CAMS, 5, "<yfov>{}</yfov>".format(math.degrees(camera.angle)))  # TODO: Review
			self.writel(S_CAMS, 5, "<aspect_ratio>{}</aspect_ratio>".format(self.scene.render.resolution_x /
			...
			self.writel(S_CAMS, 5, "<xmag>{}</xmag>".format(camera.ortho_scale * 0.5))  # TODO: Review
			self.writel(S_CAMS, 5, "<aspect_ratio>{}</aspect_ratio>".format(self.scene.render.resolution_x /
				self.scene.render.resolution_y))
			self.writel(S_CAMS, 5, "<znear>{}</znear>".format(camera.clip_start))
			self.writel(S_CAMS, 5, "<zfar>{}</zfar>".format(camera.clip_end))
	...
	def export_empty_node(self, node, il):
		...
		self.writel(S_NODES, 6,
			"<empty_draw_type>{}</empty_draw_type>".format(node.empty_display_type))
	...
	def export_curve(self, curve):
		...
		self.writel(S_GEOM, 4, "<float_array id=\"{}-positions-array\" "
			"count=\"{}\">{}</float_array>".format(splineid, len(points), position_values))
		self.writel(S_GEOM, 4, "<technique_common>")
		...
		self.writel(S_GEOM, 4, "<float_array id=\"{}-intangents-array\" "
			"count=\"{}\">{}</float_array>".format(splineid, len(points), intangent_values))
		self.writel(S_GEOM, 4, "<technique_common>")
		self.writel(S_GEOM, 5, "<accessor source=\"#{}-intangents-array\" "
			"count=\"{}\" stride=\"3\">".format(splineid, len(points) / 3))
		self.writel(S_GEOM, 6, "<param name=\"X\" type=\"float\"/>")
		self.writel(S_GEOM, 6, "<param name=\"Y\" type=\"float\"/>")
		self.writel(S_GEOM, 6, "<param name=\"Z\" type=\"float\"/>")
		...
		self.writel(S_GEOM, 4, "<float_array id=\"{}-outtangents-array\" "
			"count=\"{}\">{}</float_array>".format(splineid, len(points), outtangent_values))
		self.writel(S_GEOM, 4, "<technique_common>")
		self.writel(S_GEOM, 5, "<accessor source=\"#{}-outtangents-array\" "
			"count=\"{}\" stride=\"3\">".format(splineid, len(points) / 3))
		self.writel(S_GEOM, 6, "<param name=\"X\" type=\"float\"/>")
		self.writel(S_GEOM, 6, "<param name=\"Y\" type=\"float\"/>")
		self.writel(S_GEOM, 6, "<param name=\"Z\" type=\"float\"/>")
		...
		self.writel(S_GEOM, 4, "<Name_array id=\"{}-interpolations-array\" "
			"count=\"{}\">{}</Name_array>"
			.format(splineid, len(interps), interpolation_values))
		self.writel(S_GEOM, 4, "<technique_common>")
		self.writel(S_GEOM, 5, "<accessor source=\"#{}-interpolations-array\" "
		...
		self.writel(S_GEOM, 4, "<technique_common>")
		self.writel(S_GEOM, 5, "<accessor source=\"#{}-tilts-array\" "
		...
		self.writel(S_GEOM, 4,
			...
			.format(splineid))
		self.writel(S_GEOM, 4, "<input semantic=\"OUT_TANGENT\" "
			"source=\"#{}-outtangents\"/>".format(splineid))
		self.writel(S_GEOM, 4, "<input semantic=\"INTERPOLATION\" "
			"source=\"#{}-interpolations\"/>".format(splineid))
		self.writel(S_GEOM, 4, "<input semantic=\"TILT\" source=\"#{}-tilts\"/>"
			.format(splineid))
		self.writel(S_GEOM, 3, "</control_vertices>")
	...
	def export_animation_transform_channel(self, target, keys, matrices=True):
		...
		self.writel(S_ANIM, 3, "<float_array id=\"{}-input-array\" "
			"count=\"{}\">{}</float_array>".format(anim_id, frame_total, source_frames))
		self.writel(S_ANIM, 3, "<technique_common>")
		self.writel(S_ANIM, 4, "<accessor source=\"#{}-input-array\" "
		...
		if (matrices):
			...
			self.writel(S_ANIM, 3, "<float_array id=\"{}-transform-output-array\" "
				"count=\"{}\">{}</float_array>".format(anim_id, frame_total * 16, source_transforms))
			self.writel(S_ANIM, 3, "<technique_common>")
			self.writel(S_ANIM, 4, "<accessor source=\"#{}-transform-output-array\" count=\"{}\" "
			...
			self.writel(S_ANIM, 2, "<source id=\"{}-transform-output\">".format(anim_id))
			self.writel(S_ANIM, 3, "<float_array id=\"{}-transform-output-array\" "
				"count=\"{}\">{}</float_array>".format(anim_id, frame_total, source_transforms))
			self.writel(S_ANIM, 3, "<technique_common>")
			self.writel(S_ANIM, 4, "<accessor source=\"#{}-transform-output-array\" "
				"count=\"{}\" stride=\"1\">".format(anim_id, frame_total))
		...
		self.writel(S_ANIM, 3, "<Name_array id=\"{}-interpolation-output-array\" "
			"count=\"{}\">{}</Name_array>".format(anim_id, frame_total, source_interps))
		self.writel(S_ANIM, 3, "<technique_common>")
		self.writel(S_ANIM, 4, "<accessor source=\"#{}-interpolation-output-array\" "
			"count=\"{}\" stride=\"1\">".format(anim_id, frame_total))
		...
		self.writel(S_ANIM, 3, "<input semantic=\"OUTPUT\" "
			"source=\"#{}-transform-output\"/>".format(anim_id))
		self.writel(S_ANIM, 3, "<input semantic=\"INTERPOLATION\" "
			"source=\"#{}-interpolation-output\"/>".format(anim_id))
		self.writel(S_ANIM, 2, "</sampler>")
		if (matrices):
			self.writel(S_ANIM, 2, "<channel source=\"#{}-sampler\" "
			...
			for node in self.scene.objects:
				...
				if (allowed is not None and not (node in allowed)):
					if (node.type == "MESH" and node.data is not None and
						(node in self.armature_for_morph) and (self.armature_for_morph[node] in allowed)):
						pass
					else:
						continue
				if (node.type == "MESH" and node.data is not None and
					node.data.shape_keys is not None and (node.data in self.mesh_cache) and len(node.data.shape_keys.key_blocks) and self.config["use_shape_key_export"]):
	...
	def export_animations(self):
		...
		if (self.config["use_anim_action_all"] and len(self.skeletons)):
			...
			for x in bpy.data.actions[:]:
				...
				self.writel(S_ANIM_CLIPS, 1, "<animation_clip name=\"{}\" "
					"start=\"{}\" end=\"{}\">".format(x.name, start, end))
				for z in tcn:
					self.writel(S_ANIM_CLIPS, 2, "<instance_animation url=\"#{}\"/>".format(z))
				self.writel(S_ANIM_CLIPS, 1, "</animation_clip>")
				if (len(tcn) == 0):
					self.operator.report({"WARNING"}, "Animation clip \"{}\" contains no "
	...
	def export(self):
		...
		f.write(bytes("<COLLADA xmlns=\"http://www.collada.org/2005/11/COLLADASchema\" "
			"version=\"1.4.1\">\n", "UTF-8"))
